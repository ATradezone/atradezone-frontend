...
import Column1Skeleton from '../layout/Column1Skeleton';
import Column2Skeleton from '../layout/Column2Skeleton';
import TopbarSkeleton from '../layout/TopbarSkeleton';
...
const routeTitles: Record<string, string> = {
  ...
  '/settings/company/renewal-billing': 'Renewal & Billing',
};
...
const MainLayout = ({ children }: MainLayoutProps) => {
  ...
  // Set the document title when pathname changes
  useEffect(() => {
    if (typeof document !== 'undefined') {
      // During loading, show only the company name regardless of page type
      if (loading) {
        document.title = siteConfig.name;
      } 
      // After loading, set the full title
      else {
