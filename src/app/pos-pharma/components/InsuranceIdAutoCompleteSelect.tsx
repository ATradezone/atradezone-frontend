...
interface InsuranceIdOption {
  value: string;
  label: string;
}

// Define props interface
interface InsuranceIdAutoCompleteSelectProps {
  label?: string;
  labelAlign?: 'left' | 'center' | 'right';
  value?: string;
  onChange?: (value: string) => void;
  options: InsuranceIdOption[];
  className?: string;
  error?: string;
  disabled?: boolean;
  name?: string;
  ...
  swapActionButtonPosition?: boolean;
}

const InsuranceIdAutoCompleteSelect = ({
  label,
  labelAlign = 'left',
  value,
  onChange,
  ...
  actionButton,
  swapActionButtonPosition = false
}: InsuranceIdAutoCompleteSelectProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isFocused, setIsFocused] = useState(false);
  const [inputValue, setInputValue] = useState('');
  const [filteredOptions, setFilteredOptions] = useState<InsuranceIdOption[]>([]);
  ...
  const handleOptionSelect = (option: InsuranceIdOption) => {
    setIsSelecting(true);
  ...
  const handleInputBlur = () => {
    ...
    setTimeout(() => {
      ...
      // If value doesn't match any option, clear it
  ...
  return (
    <div className={className} ref={wrapperRef}>
      {label && (
        <label className={`block text-sm font-medium text-gray-700 mb-2 ${labelAlign === 'right' ? 'text-right' : labelAlign === 'center' ? 'text-center' : 'text-left'}`}>
          {label}
        </label>
      )}
      <div className="relative pr-8 text-gray-500">
        <input
          ...
          required={required}
          id={id}
          className={`w-full px-4 py-0.7rem border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
            error ? 'border-red-500' : 'border-gray-300'
          } ${disabled ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`}
          style={{ paddingTop: '0.5rem', paddingBottom: '0.5rem', fontFamily: 'Afacad, sans-serif', fontSize: '16px', color: '#7d7d7d' }}
        />
        ...
        {isOpen && !disabled && (
          <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto">
            {filteredOptions.length === 0 ? (
              <div className="px-4 py-2 text-gray-500 flex items-center" style={{ color: '#7d7d7d', textAlign: 'left' }}>
                <SafetyCertificateOutlined className="mr-2 text-gray-500" />
                No Insurance ID found
              </div>
            ) : (
              filteredOptions.map((option, index) => (
                <div
                  ...
                  style={{ color: '#7d7d7d', textAlign: 'left' }}
                >
                  <SafetyCertificateOutlined className="mr-2 text-gray-500" />
                  {option.label}
                </div>
...
export default InsuranceIdAutoCompleteSelect;