...
interface MedicineAutoCompleteSelectProps {
  label?: string;
  value?: string;
...
const MedicineAutoCompleteSelect = ({
  label,
  value,
  onChange,
  options,
...
}: MedicineAutoCompleteSelectProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isFocused, setIsFocused] = useState(false);
  const [inputValue, setInputValue] = useState('');
  const [filteredOptions, setFilteredOptions] = useState<MedicineOption[]>([]);
  ...
  const handleOptionSelect = (option: MedicineOption) => {
    setIsSelecting(true);
            ...
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((option, index) => (
                <div
                  ...
                  style={{ color: '#7d7d7d' }}
                >
                  <MedicineBoxOutlined className="mr-2 text-gray-500" />
                  {option.label}
                </div>
              ))
            )}
...
export default MedicineAutoCompleteSelect;