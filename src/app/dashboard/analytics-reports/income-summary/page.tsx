...
import { BarChartOutlined, CheckCircleOutlined, ClockCircleOutlined, ExclamationCircleOutlined, SearchOutlined, FilterOutlined, PlusOutlined } from '@ant-design/icons';
...
const IncomeSummaryPage = () => {
  ...
  const incomeRecords: IncomeRecord[] = [
    { id: 1, date: '2023-06-15', source: 'Product Sales', category: 'Retail', amount: 1250.50, paymentMethod: 'Credit Card', status: 'received' },
    { id: 2, date: '2023-06-14', source: 'Wholesale Orders', category: 'B2B', amount: 2450.75, paymentMethod: 'Bank Transfer', status: 'received' },
    { id: 3, date: '2023-06-14', source: 'Online Sales', category: 'E-commerce', amount: 890.25, paymentMethod: 'PayPal', status: 'received' },
    { id: 4, date: '2023-06-13', source: 'Product Sales', category: 'Retail', amount: 675.80, paymentMethod: 'Cash', status: 'received' },
    { id: 5, date: '2023-06-12', source: 'Subscription Revenue', category: 'Recurring', amount: 299.99, paymentMethod: 'Credit Card', status: 'pending' },
    { id: 6, date: '2023-06-12', source: 'Wholesale Orders', category: 'B2B', amount: 1875.30, paymentMethod: 'Bank Transfer', status: 'received' },
    { id: 7, date: '2023-06-11', source: 'Product Sales', category: 'Retail', amount: 925.60, paymentMethod: 'Credit Card', status: 'received' },
    { id: 8, date: '2023-06-10', source: 'Online Sales', category: 'E-commerce', amount: 750.40, paymentMethod: 'PayPal', status: 'overdue' },
  ];
  ...
  const getStatusDisplay = (status: 'received' | 'pending' | 'overdue') => {
    const statusConfig = {
      received: { 
  ...
  const sortedIncomeRecords = [...filteredIncomeRecords];
  if (sortConfig !== null) {
    sortedIncomeRecords.sort((a, b) => {
      if (a[sortConfig.key as keyof IncomeRecord] < b[sortConfig.key as keyof IncomeRecord]) {
  ...
  // Pagination
  const totalPages = Math.ceil(sortedIncomeRecords.length / itemsPerPage);
  const currentIncomeRecords = sortedIncomeRecords.slice(
  ...
  const columns = [
    ...
    },
    {
      key: 'status',
      title: 'Status',
      render: (_: unknown, record: IncomeRecord) => getStatusDisplay(record.status),
    },
    {
      key: 'actions',
      title: 'Actions',
      render: (_: unknown, record: IncomeRecord) => (
        <ActionButtons 
          onView={() => {
  ...
  const overdueIncome = incomeRecords
  ...
  const breadcrumbItems = [
    ...
    { name: 'Income Summary', current: true }
  ];
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Income Summary</h1>
          <div className="mt-2">
        ...
        <div 
          ...
          onClick={() => {
            // TODO: Implement add new income record functionality
          }}
          title="Add new income record"
        >
      ...
      <GenericTable 
        ...
        pagination={{
          ...
          itemsPerPage,
          totalItems: sortedIncomeRecords.length
        }}
      />
    </div>
  );
