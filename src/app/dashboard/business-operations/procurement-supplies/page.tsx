...
const ProcurementSuppliesPage = () => {
  const [procurements, setProcurements] = useState<Procurement[]>([
    { id: 1, supplier: 'MediSupply Corp', orderNumber: '#PO-001', date: '2023-06-15', total: 'RWF 25,500', status: 'Delivered' },
    { id: 2, supplier: 'PharmaDistributors Ltd', orderNumber: '#PO-002', date: '2023-06-16', total: 'RWF 18,200', status: 'Processing' },
    { id: 3, supplier: 'Global Medical Supplies', orderNumber: '#PO-003', date: '2023-06-17', total: 'RWF 32,750', status: 'Shipped' },
    { id: 4, supplier: 'HealthCare Providers Inc', orderNumber: '#PO-004', date: '2023-06-18', total: 'RWF 15,300', status: 'Pending' },
  ]);
  ...
  const breadcrumbItems = [
    ...
    { name: 'Procurement & Supplies', current: true }
  ];
  ...
  // Filter and sort procurements based on search, status, and sorting config
  const filteredAndSortedProcurements = procurements.filter(procurement => {
    ...
    const matchesStatus = filterStatus === 'all' || procurement.status.toLowerCase().includes(filterStatus);
  ...
  // Pagination
  const totalPages = Math.ceil(filteredAndSortedProcurements.length / itemsPerPage);
  const currentProcurements = filteredAndSortedProcurements.slice(
  ...
  // Stats data
  const stats = [
    { title: "Total Procurements", value: procurements.length, icon: <SearchOutlined className="w-5 h-5 text-[#2463EB]" />, color: "bg-[#F6F9FF] border border-[#DBE9FE]" },
    { title: "Pending Orders", value: procurements.filter(p => p.status === 'Pending').length, icon: <SearchOutlined className="w-5 h-5 text-yellow-500" />, color: "bg-yellow-50" },
    { title: "Processing", value: procurements.filter(p => p.status === 'Processing').length, icon: <SearchOutlined className="w-5 h-5 text-blue-500" />, color: "bg-blue-50" },
    { title: "Completed", value: procurements.filter(p => p.status === 'Delivered').length, icon: <SearchOutlined className="w-5 h-5 text-green-500" />, color: "bg-green-50" }
  ];
  ...
  const columns = [
    ...
    },
    {
      key: 'actions',
      title: 'ACTION',
      render: (_: unknown, record: Procurement) => (
        <ActionButtons 
          onView={() => console.log('View procurement', record.id)}
          onEdit={() => console.log('Edit procurement', record.id)}
          onDelete={() => console.log('Delete procurement', record.id)}
        />
      )
    }
  ];
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          {/* Add New Procurement Icon Skeleton */}
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Procurement & Supplies</h1>
          <div className="mt-2">
            <Breadcrumb items={breadcrumbItems} />
          </div>
        </div>
        
        {/* Add New Procurement Icon */}
        <div 
          ...
          onClick={() => {
            // TODO: Implement add new procurement functionality
          }}
      ...
      <div className="border-t border-[#dddddd] pt-6">
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <div className="relative mb-6 mr-0" style={{marginRight: '2.7rem' }}>
            ...
            <input
              type="text"
              placeholder="Search or filter procurements"
              value={searchTerm}
      ...
      <GenericTable 
        ...
        pagination={{
          ...
          itemsPerPage,
          totalItems: filteredAndSortedProcurements.length
        }}
      />
    </div>
  );
...
export default ProcurementSuppliesPage;